 ...
 * 808 Workspace Tests
 *
 * Tests for /808:new-workspace, /808:list-workspaces, /808:remove-workspace
 * init functions and integration with 808-tools routing.
 */
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('808-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via 808-tools ──────────────────────────────────────
...
// ─── cmdInitListWorkspaces via 808-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, '808-workspaces');
...
// ─── cmdInitRemoveWorkspace via 808-tools ───────────────────────────────────
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('808-ws-integ-');
...
describe('workspace command files', () => {
  ...
  test('new-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/808/new-workspace.md'), 'utf8');
    assert.ok(content.includes('name: 808:new-workspace'));
  ...
  test('list-workspaces command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/808/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('name: 808:list-workspaces'));
    assert.ok(content.includes('workflows/list-workspaces.md'));
  });

  test('remove-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/808/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('name: 808:remove-workspace'));
    assert.ok(content.includes('workflows/remove-workspace.md'));
  });

  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, '808/workflows/new-workspace.md'), 'utf8');
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, '808/workflows/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('init list-workspaces'));
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, '808/workflows/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('init remove-workspace'));
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in 808-tools ───────────────────────────────────────────────────

describe('workspace routing in 808-tools', () => {
  test('init new-workspace is routed correctly', () => {
    const toolsContent = fs.readFileSync(
      path.join(__dirname, '..', '808', 'bin', '808-tools.cjs'),
      'utf8'
    );
