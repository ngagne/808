...
// Sample Claude agent frontmatter (matches actual 808 agent format)
const SAMPLE_AGENT = `---
name: 808-executor
...
describe('OpenCode agent conversion (isAgent: true)', () => {
  test('keeps name: field for agents', () => {
    ...
    assert.ok(frontmatter.includes('name: 808-executor'), 'name: should be preserved for agents');
  });
  ...
  test('strips skills: array', () => {
    ...
    assert.ok(!frontmatter.includes('808-executor-workflow'), 'skill entries should be stripped');
  });
  ...
  test('keeps description: field', () => {
    const result = convertClaudeToOpencodeFrontmatter(SAMPLE_AGENT, { isAgent: true });
    const frontmatter = result.split('---')[1];
    assert.ok(frontmatter.includes('description: Executes 808 plans'), 'description should be kept');
  });

  test('preserves body content', () => {
    const result = convertClaudeToOpencodeFrontmatter(SAMPLE_AGENT, { isAgent: true });
    assert.ok(result.includes('<role>'), 'body should be preserved');
    assert.ok(result.includes('You are a 808 plan executor.'), 'body content should be intact');
  });
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: 808-codebase-mapper'), 'keeps name');
    ...
    assert.ok(!frontmatter.includes('808-mapper-workflow'), 'drops skills list items');
