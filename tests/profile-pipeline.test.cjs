...
const os = require('os');
const { run808Tools, createTempDir, createTempProject, cleanup } = require('./helpers.cjs');
...
describe('scan-sessions command', () => {
  ...
  test('returns empty array for empty sessions directory', () => {
    ...
    const result = run808Tools(`scan-sessions --path ${sessionsDir} --raw`, tmpDir);
...
describe('extract-messages command', () => {
  ...
  test('extracts user messages from synthetic session', () => {
    ...
    const result = run808Tools(`extract-messages my-project --path ${sessionsDir} --raw`, tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
  ...
  test('filters out meta and internal messages', () => {
    ...
    const result = run808Tools(`extract-messages filter-test --path ${sessionsDir} --raw`, tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
...
describe('profile-questionnaire command', () => {
  ...
  test('returns questionnaire structure', () => {
    const result = run808Tools('profile-questionnaire --raw', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
