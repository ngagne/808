...
const commandPath = path.join(repoRoot, 'commands', '808', 'milestone-summary.md');
const workflowPath = path.join(repoRoot, '808', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/808/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: 808:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary workflow', () => {
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via 808-tools'
    );
  });
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), '808-ms-test-'));
  });
