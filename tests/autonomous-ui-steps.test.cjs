...
const WORKFLOW_PATH = path.join(__dirname, '..', '808', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('UI design contract step invokes 808:ui-phase skill', () => {
      assert.ok(
        content.includes('skill="808:ui-phase"'),
        'should invoke 808:ui-phase via Skill()'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step invokes 808:ui-review skill', () => {
      assert.ok(
        content.includes('skill="808:ui-review"'),
        'should invoke 808:ui-review via Skill()'
      );
    });
