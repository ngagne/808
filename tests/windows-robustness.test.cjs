...
describe('@file: handoff in workflows', () => {
  test('all workflows calling 808-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls 808-tools.cjs init
      if (/INIT=\$\(node.*808-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling 808-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
