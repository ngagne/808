...
const commandPath = path.join(repoRoot, 'commands', '808', 'forensics.md');
const workflowPath = path.join(repoRoot, '808', 'workflows', 'forensics.md');

describe('forensics command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/808/forensics.md should exist');
  });

  test('command has correct frontmatter', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: 808:forensics'), 'should have correct command name');
...
describe('forensics fixture-based tests', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), '808-forensics-test-'));
  });
