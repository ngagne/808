/**
 * 808 Tools Tests - Antigravity Install Plumbing
 *
...
describe('convertClaudeToAntigravityContent', () => {
  ...
  describe('local install path replacements', () => {
    test('replaces ~/.claude/ with .agent/ for local installs', () => {
      const input = 'See ~/.claude/808/';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/808/'), result);
      assert.ok(!result.includes('~/.claude/'), result);
    });

    test('replaces ./.claude/ with ./.agent/', () => {
      const input = 'path ./.claude/hooks/808-check-update.js';
      const result = convertClaudeToAntigravityContent(input, false);
    ...
    test('replaces .claude/ with .agent/', () => {
      const input = 'node .claude/hooks/808-statusline.js';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/hooks/808-statusline.js'), result);
      assert.ok(!result.includes('.claude/'), result);
    });
  });

  describe('command name conversion', () => {
    test('converts /808:command to /808-command', () => {
      const input = 'Run /808:new-project to start';
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(result.includes('/808-new-project'), result);
      assert.ok(!result.includes('808:'), result);
    });

    test('converts all 808: references', () => {
      const input = '/808:plan-phase and /808:execute-phase';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('/808-plan-phase'), result);
      assert.ok(result.includes('/808-execute-phase'), result);
    });
  });
...
description: Initialize a new 808 project with requirements and roadmap
...
Initialize new project at ~/.claude/808/workflows/new-project.md
`;

  test('produces name and description only in frontmatter', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, '808-new-project', false);
    assert.ok(result.startsWith('---\n'), result);
    assert.ok(result.includes('name: 808-new-project'), result);
    assert.ok(result.includes('description: Initialize a new 808 project'), result);
  ...
  test('uses provided skillName for name field', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, '808-custom-name', false);
    assert.ok(result.includes('name: 808-custom-name'), result);
  });

  test('converts 808: command references in body', () => {
    const content = `---
name: test
description: test skill
---
Run /808:new-project to get started.
`;
    const result = convertClaudeCommandToAntigravitySkill(content, '808-test', false);
    assert.ok(result.includes('/808-new-project'), result);
    assert.ok(!result.includes('808:'), result);
  });
  ...
  test('returns unchanged content when no frontmatter', () => {
    ...
    const result = convertClaudeCommandToAntigravitySkill(noFm, '808-test', false);
...
Execute plans from ~/.claude/808/workflows/execute-phase.md
`;

  test('preserves name and description', () => {
    const result = convertClaudeAgentToAntigravityAgent(claudeAgent, false);
    assert.ok(result.includes('name: 808-executor'), result);
    assert.ok(result.includes('description: Executes 808 plans'), result);
  });
  ...
  test('uses global path for global installs', () => {
    ...
    assert.ok(result.includes('~/.gemini/antigravity/808/'), result);
  });
...
describe('copyCommandsAsAntigravitySkills', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('808-ag-test-');
    srcDir = path.join(tmpDir, 'commands', '808');
...
`);
  ...
  test('creates skills directory', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, '808', false);
    assert.ok(fs.existsSync(skillsDir));
  });

  test('creates one skill directory per command with SKILL.md', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, '808', false);
    const skillDir = path.join(skillsDir, '808-new-project');
  ...
  test('handles subdirectory commands with prefixed names', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, '808', false);
    const subSkillDir = path.join(skillsDir, '808-subdir-sub-command');
  ...
  test('SKILL.md has minimal frontmatter (name + description only)', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, '808', false);
    const content = fs.readFileSync(path.join(skillsDir, '808-new-project', 'SKILL.md'), 'utf8');
    assert.ok(content.includes('name: 808-new-project'), content);
  ...
  test('SKILL.md body has paths converted for local install', () => {
    ...
    const content = fs.readFileSync(path.join(skillsDir, '808-new-project', 'SKILL.md'), 'utf8');
    // 808: → 808- conversion
    assert.ok(!content.includes('808:'), content);
  });

  test('removes old 808-* skill dirs before reinstalling', () => {
    // Create a stale skill dir
    const staleDir = path.join(skillsDir, '808-old-skill');
  ...
  test('does not remove non-808 skill dirs', () => {
    // Create a non-808 skill dir
    const otherDir = path.join(skillsDir, 'my-custom-skill');
    fs.mkdirSync(otherDir, { recursive: true });

    copyCommandsAsAntigravitySkills(srcDir, skillsDir, '808', false);

    assert.ok(fs.existsSync(otherDir), 'non-808 skill dir should be preserved');
  });
});
...
describe('writeManifest (Antigravity)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('808-manifest-ag-');
    // Create minimal structure
    const skillsDir = path.join(tmpDir, 'skills', '808-help');
    ...
    fs.writeFileSync(path.join(agent808Dir, 'VERSION'), '1.0.0');
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    fs.writeFileSync(path.join(agentsDir, '808-executor.md'), '---\nname: 808-executor\n---\n');
  });
  ...
  test('manifest includes skills in skills/ directory', () => {
    writeManifest(tmpDir, 'antigravity');
    const manifest = JSON.parse(fs.readFileSync(path.join(tmpDir, '808-file-manifest.json'), 'utf8'));
