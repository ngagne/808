/**
 * 808 Tools Tests - Dispatcher
 *
 * Tests for 808-tools.cjs dispatch routing and error paths.
...
describe('dispatcher error paths', () => {
  ...
  test('no-command invocation prints usage and exits non-zero', () => {
    const result = run808Tools('', tmpDir);
  ...
  test('unknown command produces clear error and exits non-zero', () => {
    const result = run808Tools('nonexistent-cmd', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd= form overrides working directory', () => {
    ...
    const result = run808Tools(`--cwd=${tmpDir} state load`, process.cwd());
  ...
  test('--cwd= with empty value produces error', () => {
    const result = run808Tools('--cwd= state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd with invalid path produces error', () => {
    const result = run808Tools('--cwd /nonexistent/path/xyz state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('template unknown subcommand errors', () => {
    const result = run808Tools('template bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('frontmatter unknown subcommand errors', () => {
    const result = run808Tools('frontmatter bogus file.md', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('verify unknown subcommand errors', () => {
    const result = run808Tools('verify bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phases unknown subcommand errors', () => {
    const result = run808Tools('phases bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('roadmap unknown subcommand errors', () => {
    const result = run808Tools('roadmap bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('requirements unknown subcommand errors', () => {
    const result = run808Tools('requirements bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phase unknown subcommand errors', () => {
    const result = run808Tools('phase bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('milestone unknown subcommand errors', () => {
    const result = run808Tools('milestone bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('validate unknown subcommand errors', () => {
    const result = run808Tools('validate bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('todo unknown subcommand errors', () => {
    const result = run808Tools('todo bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('uat unknown subcommand errors', () => {
    const result = run808Tools('uat bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('init unknown workflow errors', () => {
    const result = run808Tools('init bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
...
describe('dispatcher routing branches', () => {
  ...
  test('find-phase locates phase directory by number', () => {
    ...
    const result = run808Tools('find-phase 01', tmpDir);
  ...
  test('init resume returns valid JSON', () => {
    ...
    const result = run808Tools('init resume', tmpDir);
  ...
  test('init verify-work returns valid JSON', () => {
    ...
    const result = run808Tools('init verify-work 01', tmpDir);
  ...
  test('roadmap update-plan-progress updates phase progress', () => {
    ...
    const result = run808Tools('roadmap update-plan-progress 1', tmpDir);
  ...
  test('state with no subcommand calls cmdStateLoad', () => {
    ...
    const result = run808Tools('state', tmpDir);
