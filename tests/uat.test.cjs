...
describe('audit-uat command', () => {
  ...
  test('returns empty results when no UAT files exist', () => {
    ...
    const result = run808Tools('audit-uat --raw', tmpDir);
...
`);
    ...
    const result = run808Tools(['uat', 'render-checkpoint', '--file', '.planning/phases/01-test-phase/01-UAT.md', '--raw'], tmpDir);
...
`);
    ...
    const result = run808Tools(['uat', 'render-checkpoint', '--file', '.planning/phases/01-test-phase/01-UAT.md'], tmpDir);
