/**
 * 808 Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('808-execute-phase');
  ...
  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('808-discuss-phase');
...
You are a 808 plan executor.
    ...
    assert.ok(result.includes('"808-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes 808 plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: 808-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes 808 plans with atomic commits'), 'purpose from description');
...
Run /808:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$808-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/808:execute-phase'), 'original slash command removed');
  });
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('808-executor', sampleAgent);
    assert.ok(result.includes('name = "808-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('808-unknown', minimalAgent);
    assert.ok(result.includes('name = "808-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "808 agent 808-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('808-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      '808-executor', '808-planner', '808-phase-researcher',
      '808-project-researcher', '808-research-synthesizer', '808-verifier',
      '808-codebase-mapper', '808-roadmapper', '808-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['808-plan-checker', '808-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: '808-executor', description: 'Executes plans' },
    { name: '808-planner', description: 'Creates plans' },
  ];
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.808-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.808-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.808-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/808-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/808-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/808-planner.toml"'), 'absolute planner path');
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), '808-codex-merge-'));
  });
  ...
  test('case 2: replaces existing 808 block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: '808-executor', description: 'Updated description' },
      { name: '808-planner', description: 'New agent' },
    ]);
  ...
  test('case 3: appends to config without 808 marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
  ...
  test('case 3 with existing [features]: preserves user features, does not inject 808 keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
  ...
  test('case 3 strips existing [agents.808-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
  ...
  test('case 2 strips leaked [agents] and [agents.808-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
  ...
  test('case 2 strips leaked 808-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.808-executor\]\s*$/gm), 0, 'removes leaked 808 agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.808-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked 808 sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.808-executor\]\s*$/gm), 0, 'removes leaked 808 agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.808-executor\]\s*$/gm), 1, 'keeps one managed agent section in the 808 block');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending 808 block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
  ...
  test('uses the first newline style when appending 808 block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), '808-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, '808-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, '808-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, '808-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "808-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes 808 plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, '808-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "808-plan-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), '808-codex-e2e-'));
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# 808 Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes 808 SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'writes one 808 update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'adds one 808 update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. 808 must not prepend [features] before
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.808-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'does not duplicate 808 update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'keeps one 808 update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'remains idempotent for the 808 hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'adds one 808 update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 0, 'does not add the 808 hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.808-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'adds one 808 update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-808 hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-808 hook exactly once');
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'keeps one 808 update hook');
    assertUsesOnlyEol(content, '\r\n');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'adds the 808 update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# 808 Agent Configuration — managed by 808 installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# 808 Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the 808 hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /808-update-check\.js/g), 1, 'does not duplicate the 808 hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), '808-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-808 hooks');
    assert.strictEqual(countMatches(cleaned, /808-update-check\.js/g), 0, 'removes only the 808 update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.808-/g), 0, 'removes managed 808 agent sections');
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing 808 hook state', () => {
