...
- `--path` — Target directory. Defaults to `~/808-workspaces/<name>`
...
<objective>
Create a physical workspace directory containing copies of specified git repos (as worktrees or clones) with an independent `.planning/` directory for isolated 808 sessions.

**Use cases:**
- Multi-repo orchestration: work on a subset of repos in parallel with isolated 808 state
...
**After this command:** `cd` into the workspace and run `/808:new-project` to initialize 808.
</objective>

<execution_context>
@~/.claude/808/workflows/new-workspace.md
@~/.claude/808/references/ui-brand.md
</execution_context>

<process>
Execute the new-workspace workflow from @~/.claude/808/workflows/new-workspace.md end-to-end.
