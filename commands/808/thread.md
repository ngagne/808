...
```
No threads found. Create one with: /808:thread <description>
```
</mode_list>
...
5. Commit:
   ```bash
   node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /808:thread {slug}
   ```
</mode_create>
...
- Lighter weight than /808:pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /808:add-phase or /808:add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other 808 structures
</notes>
