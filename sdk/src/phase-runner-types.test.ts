...
describe('Phase lifecycle types', () => {
  ...
  // ─── 808EventType phase lifecycle values ───────────────────────────────
  ...
  describe('PhaseOpInfo interface', () => {
    ...
    it('matches the documented init phase-op JSON shape', () => {
      // Simulate parsing JSON from 808-tools.cjs
...
// ─── 808Tools typed methods ──────────────────────────────────────────────────

describe('808Tools typed methods', () => {
  let tmpDir: string;
  let fixtureDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `808-tools-phase-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('initPhaseOp()', () => {
    it('returns typed PhaseOpInfo from 808-tools output', async () => {
  ...
  describe('configGet()', () => {
    it('returns string value from 808-tools config', async () => {
      const scriptPath = await createScript(
        'config-get.cjs',
        `
        const args = process.argv.slice(2);
