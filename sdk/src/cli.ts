#!/usr/bin/env node
/**
 * CLI entry point for 808-sdk.
 *
 * Usage: 808-sdk run "<prompt>" [--project-dir <dir>] [--ws-port <port>]
...
export const USAGE = `
Usage: 808-sdk <command> [args] [options]
...
async function readStdin(): Promise<string> {
  ...
  if (stdin.isTTY) {
    throw new Error(
      ...
      '  808-sdk init @path/to/prd.md\n' +
      '  808-sdk init "build a todo app"\n' +
      '  cat prd.md | 808-sdk init'
    );
  }
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (args.version) {
    ...
    console.log(`808-sdk v${ver}`);
    return;
  }

  if (args.command !== 'run' && args.command !== 'init' && args.command !== 'auto') {
    console.error('Error: Expected "808-sdk run <prompt>", "808-sdk auto", or "808-sdk init [input]"');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'run' && !args.prompt) {
    console.error('Error: "808-sdk run" requires a prompt');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'init') {
    ...
    // Build 808 instance for tools and event stream
    const app = new 808({
  ...
  try {
    const result = await app.run(args.prompt!);
