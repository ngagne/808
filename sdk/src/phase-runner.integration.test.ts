/**
 * Integration test — proves PhaseRunner state machine works against real 808-tools.cjs.
 *
 * Creates a temp `.planning/` directory structure, instantiates real 808Tools,
...
describe('Integration: PhaseRunner against real 808-tools.cjs', () => {
  ...
  // ── Test 4: 808.runPhase() public API delegates correctly ──

  it('808.runPhase() creates collaborators and delegates to PhaseRunner', { timeout: 300_000 }, async () => {
    // Import 808 here to test the public API wiring
    ...
    // Proves the full wiring works: 808 → PhaseRunner → 808Tools → 808-tools.cjs
    expect(result.phaseNumber).toBe('01');
    expect(result.phaseName).toBe('integration-test');
    expect(result.steps.length).toBeGreaterThanOrEqual(1);
...
async function createMultiWavePlanningDir(): Promise<string> {
  const tmpDir = await mkdtemp(join(tmpdir(), '808-sdk-wave-int-'));
