/**
 * 808 Tools Bridge — shells out to `808-tools.cjs` for state management.
 *
 * All `.planning/` state operations go through 808-tools.cjs rather than
...
export class Agent808Tools {
  ...
  /**
   * Execute a 808-tools command and return parsed JSON output.
   * Handles the `@file:` prefix pattern for large results.
   */
  async exec(command: string, args: string[] = []): Promise<unknown> {
    const fullArgs = [this.808ToolsPath, command, ...args];
  ...
  /**
   * Parse 808-tools output, handling `@file:` prefix.
   */
  ...
  /**
   * Execute a 808-tools command and return raw stdout without JSON parsing.
   * Use for commands like `config-set` that return plain text, not JSON.
   */
  async execRaw(command: string, args: string[] = []): Promise<string> {
    const fullArgs = [this.808ToolsPath, command, ...args, '--raw'];
  ...
  /**
   * Query phase state from 808-tools.cjs `init phase-op`.
  ...
  /**
   * Get a config value from 808-tools.cjs.
   */
  ...
  /**
   * Begin phase state tracking in 808-tools.cjs.
   */
  ...
  /**
   * Query new-project init state from 808-tools.cjs `init new-project`.
  ...
  /**
   * Set a config value via 808-tools.cjs `config-set`.
   * Handles type coercion (booleans, numbers, JSON) on the 808-tools side.
