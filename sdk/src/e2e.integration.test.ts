...
describe.skipIf(!cliAvailable)('E2E: Single plan execution', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), '808-sdk-e2e-'));
  ...
  it('executes a single plan and returns a valid PlanResult', async () => {
    ...
    const result = await app.executePlan('sample-plan.md');
    ...
    expect(outputContent).toContain('hello from 808-sdk');
  ...
  it('proves session isolation (R014) — different session IDs for sequential runs', async () => {
    ...
    const tmpDir2 = await mkdtemp(join(tmpdir(), '808-sdk-e2e-'));
...
describe.skipIf(!cliAvailable)('E2E: Event stream during plan execution (R007)', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), '808-sdk-e2e-stream-'));
    await cp(fixturesDir, tmpDir, { recursive: true });
  });
...
describe('E2E: Error handling', () => {
  it('returns failure for nonexistent plan path', async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), '808-sdk-e2e-err-'));

    try {
      const app = new 808({ projectDir: tmpDir });
      await expect(app.executePlan('nonexistent-plan.md')).rejects.toThrow();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
    }
  });
