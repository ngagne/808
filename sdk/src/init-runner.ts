/**
 * InitRunner — orchestrates the 808 new-project init workflow.
 *
 ...
 * prompts derived from 808-1 workflow/agent/template files on disk.
 */
...
export class InitRunner {
  ...
  async run(input: string): Promise<InitResult> {
    ...
    try {
      ...
      const configResult = await this.runStep('config', async () => {
        ...
        // Persist auto_advance via 808-tools (validates & updates state)
  ...
  private async buildProjectPrompt(input: string): Promise<string> {
    const template = await this.read808File('templates/project.md');
  ...
  private async buildResearchPrompt(
    ...
    const agentDef = await this.readAgentFile('808-project-researcher.md');
    const template = await this.read808File(`templates/research-project/${researchType}.md`);
  ...
  private async buildSynthesisPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('808-research-synthesizer.md');
    const summaryTemplate = await this.read808File('templates/research-project/SUMMARY.md');
  ...
  private async buildRequirementsPrompt(): Promise<string> {
    const reqTemplate = await this.read808File('templates/requirements.md');
  ...
  private async buildRoadmapPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('808-roadmapper.md');
    const roadmapTemplate = await this.read808File('templates/roadmap.md');
    const stateTemplate = await this.read808File('templates/state.md');
  ...
  /**
   * Read a file from the 808 templates directory.
   * Tries sdk/prompts/{relativePath} first (headless versions), then
   * falls back to 808-1 originals (~/.claude/808/).
   */
  private async read808File(relativePath: string): Promise<string> {
   ...
   * falls back to 808-1 originals (~/.claude/agents/).
   */
