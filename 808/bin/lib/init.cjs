...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, '808-phase-researcher'),
    planner_model: resolveModelInternal(cwd, '808-planner'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const firecrawlKeyFile = path.join(homedir, '.808', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.808', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, '808-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, '808-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, '808-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, '808-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /808:new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /808:new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/808:execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/808:plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/808:discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, '808-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., '808-executor', '808-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills 808-executor 2>/dev/null)
 */
