...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /808:new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  'Before using Edit, Write, or other file-changing tools, start work through a 808 command so planning artifacts and execution context stay in sync.',
  '',
  'Use these entry points:',
  '- `/808:quick` for small fixes, doc updates, and ad-hoc tasks',
  '- `/808:debug` for investigation and bug fixing',
  '- `/808:execute-phase` for planned phase work',
  '',
  'Do not make direct repo edits outside a 808 workflow unless the user explicitly asks to bypass it.',
].join('\n');
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/808:profile-user` to generate your developer profile.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- 808:${sectionName}-start`;
  const endMarker = `<!-- 808:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- 808:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- 808:${sectionName}-end -->`,
  ].join('\n');
}
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', '808', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/808:profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', '808', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/808:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by 808 from ${dataSource}. Run \`/808:profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- 808:profile-start -->';
    const endMarker = '<!-- 808:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## 808 Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- 808:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- 808:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- 808:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- 808:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /808:profile-user to unlock Developer Profile.';
