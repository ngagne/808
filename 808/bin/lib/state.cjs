...
/**
 * Write a WAITING.json signal file when 808 hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .808/WAITING.json if .808 exists).
 * Fixes #1034.
 */
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.808', 'WAITING.json'),
