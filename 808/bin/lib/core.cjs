...
/**
 * Remove stale 808-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., '808-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = '808-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `808-${Date.now()}.json`);
 ...
 * Applied at write points so 808-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
...
/**
 * Resolve the agents directory from the 808 install location.
 * 808-tools.cjs lives at <configDir>/808/bin/808-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  // __dirname is 808/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents');
}

/**
 * Check which 808 agents are installed on disk.
