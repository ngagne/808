<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /808:plan-phase --gaps.
...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init verify-work "${PHASE_ARG}")
...
Provide a phase number to start testing (e.g., /808:verify-work 4)
```
...
```bash
CHECKPOINT=$(node "$HOME/.claude/808/bin/808-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- `/808:plan-phase {next}` — Plan next phase
- `/808:execute-phase {next}` — Execute next phase
- `/808:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @~/.claude/808/workflows/diagnose-issues.md
...
Spawn 808-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /808:execute-phase
...
Spawn 808-planner with revision context:
...
3. Abandon (exit, user runs /808:plan-phase manually)
...
`/clear` then `/808:execute-phase {phase} --gaps-only`
...
- [ ] If issues: 808-planner creates fix plans (gap_closure mode)
- [ ] If issues: 808-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/808:execute-phase --gaps-only` when complete
</success_criteria>
