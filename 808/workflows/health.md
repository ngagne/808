...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" validate health $REPAIR_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /808:health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /808:new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /808:health --repair
```
</step>
...
```
Would you like to run /808:health --repair to fix N issues automatically?
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" validate health
```
