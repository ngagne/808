...
<required_reading>
@~/.claude/808/references/verification-patterns.md
@~/.claude/808/templates/verification-report.md
</required_reading>
...
Use 808-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/808/bin/808-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use 808-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/808/bin/808-tools.cjs" verify artifacts "$plan")
...
Use 808-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/808/bin/808-tools.cjs" verify key-links "$plan")
...
See ~/.claude/808/templates/verification-report.md for complete template.
</step>
