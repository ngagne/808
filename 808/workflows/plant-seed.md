...
Seeds auto-surface during /808:new-milestone when trigger conditions match the
...
This seed should be presented during `/808:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /808:new-milestone
