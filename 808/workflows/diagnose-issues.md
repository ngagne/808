...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-debugger 2>/dev/null)
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
