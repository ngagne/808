...
- 808-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-assumptions-analyzer 2>/dev/null)
```
...
Use /808:progress to see available phases.
```
Exit workflow.
...
Spawn a `808-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="808-assumptions-analyzer", prompt="""
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Also available:**
- `/808:plan-phase ${PHASE} --skip-research` — plan without research
- `/808:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="808:plan-phase", args="${PHASE} --auto")`
