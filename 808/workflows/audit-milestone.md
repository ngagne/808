...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/808/bin/808-tools.cjs" resolve-model 808-integration-checker --raw)
```
...
node "$HOME/.claude/808/bin/808-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/808/bin/808-tools.cjs" find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="808-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/808/bin/808-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/808:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/808:validate-phase {N}` |

Phases needing validation: run `/808:validate-phase {N}` for each flagged phase.
...
- /808:complete-milestone {version} — proceed anyway (accept tech debt)
