<purpose>
Check for 808 updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether 808 is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/808:update` targets the runtime that invoked it.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/808/VERSION" ] || [ -f "./$dir/808/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/808/VERSION"
    LOCAL_MARKER_FILE="./$dir/808/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/808/VERSION" ] || [ -f "$HOME/$dir/808/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/808/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/808/workflows/update.md"
...
```bash
npm view 808 version 2>/dev/null
```
...
To update manually: `npx 808 --global`
```
...
⚠️  **Note:** The installer performs a clean install of 808 folders:
- `commands/808/` will be wiped and replaced
- `808/` will be wiped and replaced
- `agents/808-*` files will be replaced
...
- Custom commands not in `commands/808/` ✓
- Custom agents not prefixed with `808-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any 808 files directly, they'll be automatically backed up to `808-local-patches/` and can be reapplied with `/808:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y 808@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y 808@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y 808@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/808-update-check.json"
  rm -f "$HOME/$dir/cache/808-update-check.json"
done
```

The SessionStart hook (`808-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  808 Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/808-build/808/blob/main/CHANGELOG.md)
```
</step>
...
Check for 808-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /808:reapply-patches to merge your modifications into the new version.
```
