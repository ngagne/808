...
- **Claude Code:** Uses `Task(subagent_type="808-executor", ...)` — blocks until complete, returns result
...
These are the valid 808 subagent types registered in .claude/agents/ (or equivalent for your runtime).
...
- 808-debugger — Diagnoses and fixes issues
- 808-codebase-mapper — Maps project structure and dependencies
- 808-integration-checker — Checks cross-phase integration
- 808-nyquist-auditor — Validates verification coverage
- 808-ui-researcher — Researches UI/UX approaches
- 808-ui-checker — Reviews UI implementation quality
...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init execute-phase "${PHASE_ARG}")
...
Check if the current runtime is Copilot by testing for the `@808-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/808/bin/808-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/808/workflows/execute-plan.md` **inline**
...
- Maintains 808's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning 808
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/808/bin/808-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For 808-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @~/.claude/808/workflows/execute-plan.md
       @~/.claude/808/templates/summary.md
       @~/.claude/808/references/checkpoints.md
       @~/.claude/808/references/tdd.md
       </execution_context>
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a 808 or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/808/bin/808-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/808/bin/808-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/808/bin/808-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/808:progress` and `/808:audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/808:verify-work` on it.
...
```bash
COMPLETION=$(node "$HOME/.claude/808/bin/808-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/808:progress` and `/808:audit-uat`.
```

```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/808:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/808/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/808:transition` command. Never suggest it. The transition workflow is internal only.**
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not 808. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/808:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
