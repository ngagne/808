<purpose>
Interactive configuration of 808 workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.808/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" config-ensure-section
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" state load)
...
- `workflow.ui_safety_gate` — prompt to run /808:ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /808:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /808:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (808/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (808/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /808:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  }
])
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.808/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.808/defaults.json`:

```bash
mkdir -p ~/.808
```

Write `~/.808/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /808:plan-phase and /808:execute-phase runs.

Quick commands:
- /808:set-profile <profile> — switch model profile
- /808:plan-phase --research — force research
- /808:plan-phase --skip-research — skip research
- /808:plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.808/defaults.json)
