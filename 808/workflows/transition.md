...
There is no `/808:transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/808:transition`.

**Valid user commands for phase progression:**
- `/808:discuss-phase {N}` — discuss a phase before planning
- `/808:plan-phase {N}` — plan a phase
- `/808:execute-phase {N}` — execute a phase
- `/808:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/808:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to 808-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/808/bin/808-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `808-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/808/bin/808-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `808-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `808_WORKSTREAM` is not set / flat mode).
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" config-set workflow._auto_chain_active false
```
...
Do NOT suggest `/808:complete-milestone` or `/808:new-milestone`.
