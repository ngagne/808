...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel 808-codebase-mapper agents.

Use Task tool with `subagent_type="808-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `808-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="808-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `808-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/808:map-codebase`
...
- If Task tool available: 4 parallel 808-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in 808 style
</success_criteria>
