...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init progress)
...
Run /808:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/808:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/808:new-project`.
</step>

<step name="load">
**Use structured extraction from 808-tools:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap analyze)`
- `STATE=$(node "$HOME/.claude/808/bin/808-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$HOME/.claude/808/bin/808-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from 808-tools, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(node "$HOME/.claude/808/bin/808-tools.cjs" progress bar --raw)
```
...
## Pending Todos
- [count] pending — /808:check-todos to review

## Active Debug Sessions
- [count] active — /808:debug to continue
...
```bash
DEBT=$(node "$HOME/.claude/808/bin/808-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/808:discuss-phase {phase}` — gather context and clarify approach
...
**Also available:**
- `/808:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/808:plan-phase {phase}` — skip discussion, plan directly
- `/808:list-phase-assumptions {phase}` — see Claude's assumptions
...
```bash
NEXT_PHASE_SECTION=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
...
`/808:discuss-phase {Z+1}` — gather context and clarify approach
...
**Also available:**
- `/808:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/808:plan-phase {Z+1}` — skip discussion, plan directly
- `/808:verify-work {Z}` — user acceptance test before continuing
...
- [ ] Smart routing: /808:execute-phase if plans exist, /808:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate 808 command
      </success_criteria>
