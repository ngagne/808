...
```bash
CONFIG=$(node "$HOME/.claude/808/bin/808-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/808/bin/808-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /808:complete-milestone (if last phase in milestone)
- /808:progress (to see what's next)
...
- /808:complete-milestone — if all phases in milestone are done
- /808:progress — see overall project state
- /808:execute-phase {next} — continue to next phase
</offer_next>
