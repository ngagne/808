...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing 808-tools.cjs subcommands and the 808-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/808/references/ui-brand.md (display patterns)
- @$HOME/.claude/808/agents/808-user-profiler.md (profiler agent definition)
- @$HOME/.claude/808/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/808/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/808/USER-PROFILE.md" "$HOME/.claude/808/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/808/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /808:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/808/bin/808-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/808/bin/808-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn 808-user-profiler agent using Task tool:**

Use the Task tool to spawn the `808-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/808/references/user-profiling.md`
...
Reference: @$HOME/.claude/808/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/808/bin/808-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/808-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/808/bin/808-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/808-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/808/bin/808-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/808/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/808:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/808/USER-PROFILE.md" and jump to step 10.
...
**For /808:dev-preferences (if selected):**

```bash
node $HOME/.claude/808/bin/808-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /808:dev-preferences at $HOME/.claude/commands/808/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/808/bin/808-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/808/bin/808-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any 808-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/808/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 808 > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/808/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /808:dev-preferences   $HOME/.claude/commands/808/dev-preferences.md
...
- [ ] Artifacts generated sequentially via 808-tools.cjs subcommands
