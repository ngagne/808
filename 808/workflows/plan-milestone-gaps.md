<purpose>
Create all phases necessary to close gaps identified by `/808:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/808:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/808:audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/808/bin/808-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/808:execute-phase {N}` — if plans already exist
...
`/808:audit-milestone` — re-audit to verify gaps closed
`/808:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/808:plan-phase` next
</success_criteria>
