<purpose>
Remove a 808 workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/808:list-workspaces` to show available workspaces, then ask:
