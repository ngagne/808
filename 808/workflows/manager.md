...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init manager)
...
All {phase_count} phases done. Ready for final steps:
  → /808:verify-work — run acceptance testing
  → /808:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="808:verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="808:complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in 808 config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="808:discuss-phase", args="{PHASE_NUM}")
```
...
Task(
  ...
  prompt="You are running the 808 plan-phase workflow for phase {N} of the project.
...
Steps:
1. Read the plan-phase workflow: cat ~/.claude/808/workflows/plan-phase.md
2. Run: node \"$HOME/.claude/808/bin/808-tools.cjs\" init plan-phase {N}
...
5. Spawn a 808-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a 808-plan-checker subagent to verify.
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the 808 execute-phase workflow for phase {N} of the project.
...
Steps:
1. Read the execute-phase workflow: cat ~/.claude/808/workflows/execute-phase.md
2. Run: node \"$HOME/.claude/808/bin/808-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn 808-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a 808-verifier subagent if verifier is enabled.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /808:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/808:manager` or `/808:progress` invocation.
