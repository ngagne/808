...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/808:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/808:new-milestone` first."
...
```bash
ROADMAP=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 808 ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/808/bin/808-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="808:ui-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="808:plan-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="808:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="808:plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="808:ui-review", args="${PHASE_NUM}")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `808:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="808:audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /808:audit-milestone to review, then /808:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /808:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="808:complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /808:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="808:audit-milestone")
