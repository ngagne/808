<purpose>
Detect current project state and automatically advance to the next logical 808 workflow step.
...
```bash
# Get state snapshot
node "$HOME/.claude/808/bin/808-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No 808 project detected. Run `/808:new-project` to get started.
```
Exit.
</step>
...
→ Next action: `/808:discuss-phase <first-phase>`
...
▶ **Next step:** `/808:[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/808:next` is zero-friction advancement.
</step>
