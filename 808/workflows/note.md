...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/808:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/808:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a 808 project. Run `/808:new-project` to initialize one."
...
<edge_cases>
1. **"list" as note text**: `/808:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `~/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/808:new-project`
