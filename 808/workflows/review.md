...
Write to a temp file: `/tmp/808-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/808-review-prompt-{phase}.md)" 2>/dev/null > /tmp/808-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/808-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/808-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/808-review-prompt-{phase}.md)" 2>/dev/null > /tmp/808-review-codex-{phase}.md
```
...
Commit:
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/808:plan-phase --reviews)
</success_criteria>
