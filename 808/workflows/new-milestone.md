...
- Check for MILESTONE-CONTEXT.md (from /808:discuss-milestone)
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init new-milestone)
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/808:plan-phase` behavior. To change the default, use `/808:settings`.
...
Spawn 4 parallel 808-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/808/templates/research-project/{FILE}
</output>
", subagent_type="808-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
- [ ] 808-roadmapper spawned with phase numbering context
