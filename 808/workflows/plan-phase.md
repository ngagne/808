<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates 808-phase-researcher, 808-planner, and 808-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/808/bin/808-tools.cjs" agent-skills 808-checker 2>/dev/null)
```
...
**If `planning_exists` is false:** Error — run `/808:new-project` first.
...
Then re-run /808:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/808/bin/808-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 808 ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn 808-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
Task(
  ...
  subagent_type="808-phase-researcher",
...
**If found:**
1. Read template: `~/.claude/808/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/808/bin/808-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
1. Generate UI-SPEC first — Run /808:ui-phase {N} then re-run /808:plan-phase {N}
...
```bash
_808_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_808_field "$INIT" state_path)
ROADMAP_PATH=$(_808_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_808_field "$INIT" requirements_path)
RESEARCH_PATH=$(_808_field "$INIT" research_path)
VERIFICATION_PATH=$(_808_field "$INIT" verification_path)
UAT_PATH=$(_808_field "$INIT" uat_path)
CONTEXT_PATH=$(_808_field "$INIT" context_path)
REVIEWS_PATH=$(_808_field "$INIT" reviews_path)
```
...
## 8. Spawn 808-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 808 ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /808:execute-phase. Plans need:
...
## 10. Spawn 808-plan-checker Agent
...
Task(
  ...
  subagent_type="808-plan-checker",
  model="{checker_model}",
...
- /808:plan-phase {X} --research — re-research first
- /808:review --phase {X} --all — peer review plans with external AIs
- /808:plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/808:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/808:plan-phase N --skip-research
```
</windows_troubleshooting>
