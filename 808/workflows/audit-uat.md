...
```bash
AUDIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/808:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/808:verify-work {phase}`
```
</step>
