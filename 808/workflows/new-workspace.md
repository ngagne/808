...
```bash
INIT=$(node "$HOME/.claude/808/bin/808-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /808:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /808:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /808:new-project    # Initialize 808 in the workspace
```
...
**Offer to initialize 808 (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize 808"
- question: "Would you like to initialize a 808 project in the new workspace?"
- options:
  - "Yes — run /808:new-project" → tell user to `cd $TARGET_PATH` first, then run `/808:new-project`
