...
```bash
PHASE_INFO=$(node "$HOME/.claude/808/bin/808-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/808/bin/808-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.claude/808/bin/808-tools.cjs" find-phase "${PHASE}" --raw)
```
