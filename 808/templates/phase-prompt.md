...
> **Note:** Planning methodology is in `agents/808-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/808/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/808:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/808/references/tdd.md` for TDD plan structure.
...
See `~/.claude/808/templates/user-setup.md` for full schema and examples
...
See `~/.claude/808/workflows/verify-phase.md` for verification logic.
