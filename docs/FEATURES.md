...
**Command:** `/808:new-project [--auto @file.md]`
...
- Existing codebase context (from `/808:map-codebase`) is loaded if present

---

### 2. Phase Discussion

**Command:** `/808:discuss-phase [N] [--auto] [--batch]`
...
**Command:** `/808:plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: System MUST prompt user to run `/808:ui-phase` if frontend phase detected and no UI-SPEC.md exists (UI safety gate)
...
**Commands:** `/808:audit-milestone`, `/808:complete-milestone`, `/808:new-milestone [name]`
...
**Gap Closure:** `/808:plan-milestone-gaps` creates phases to close gaps identified by audit.
...
**Commands:** `/808:add-phase`, `/808:insert-phase [N]`, `/808:remove-phase [N]`
...
**Command:** `/808:quick [--full] [--discuss] [--research]`

**Purpose:** Ad-hoc task execution with 808 guarantees but a faster path.
...
**Purpose:** Analyze freeform text and route to the appropriate 808 command.

**Requirements:**
- REQ-DO-01: System MUST parse user intent from natural language input
- REQ-DO-02: System MUST map intent to the best matching 808 command
...
- REQ-NEXT-04: System MUST suggest `/808:new-project` if no project exists
- REQ-NEXT-05: System MUST suggest `/808:complete-milestone` when all phases are complete

**State Detection Logic:**
| State | Action |
|-------|--------|
| No `.planning/` directory | Suggest `/808:new-project` |
| Phase has no CONTEXT.md | Run `/808:discuss-phase` |
| Phase has no PLAN.md files | Run `/808:plan-phase` |
| Phase has plans but no SUMMARY.md | Run `/808:execute-phase` |
| Phase executed but no VERIFICATION.md | Run `/808:verify-work` |
| All phases complete | Suggest `/808:complete-milestone` |
...
- REQ-NYQ-05: System MUST support retroactive validation via `/808:validate-phase`
...
**Retroactive Validation (`/808:validate-phase [N]`):**
...
- REQ-POSTVER-03: System MUST log issues for `/808:verify-work` to address
...
**When:** Runs automatically during `/808:execute-phase` before the verifier step.
...
**When:** Runs automatically at the end of `/808:plan-phase` after the plan checker loop.
...
- REQ-CTX-06: Context monitor MUST differentiate 808-active vs non-808-active projects
...
**Commands:** `/808:pause-work`, `/808:resume-work`, `/808:progress`
...
**Command:** `/808:set-profile <quality|balanced|budget|inherit>`
...
**Command:** `/808:map-codebase [area]`

**Purpose:** Analyze an existing codebase before starting a new project, so 808 understands what exists.
...
- REQ-MAP-04: Subsequent `/808:new-project` MUST load codebase mapping and focus questions on what's being added
...
**Commands:** `/808:add-todo [desc]`, `/808:check-todos`
...
**Purpose:** Update 808 to the latest version with changelog preview.
...
- REQ-UPDATE-04: System MUST back up locally modified files to `808-local-patches/`
- REQ-UPDATE-05: `/808:reapply-patches` MUST restore local modifications after update

---

### 32. Settings Management

**Command:** `/808:settings`
...
- REQ-SETTINGS-03: System MUST support saving as global defaults (`~/.808/defaults.json`)
...
**Purpose:** Run 808 across 6 different AI coding agent runtimes.
...
- REQ-RUNTIME-05: Uninstall MUST cleanly remove all 808 files without affecting other configurations
...
**Statusline Display:**
```
[⬆ /808:update │] model │ [current task │] directory [█████░░░░░ 50%]
```
...
**Command:** `/808:profile-user [--questionnaire] [--refresh]`
...
- `808-user-profiler` agent — Behavioral analysis from session data
...
Every `/808:progress` call scans ALL phases in the current milestone for outstanding items (pending, skipped, blocked, human_needed). Displays a non-blocking warning section with actionable links.
...
**Requirements:**
- REQ-DEBT-01: System MUST surface outstanding UAT/verification items from ALL prior phases in `/808:progress`
...
- REQ-DEBT-06: `/808:audit-uat` MUST scan all phases, categorize items by testability, and produce a human test plan
...
**Command:** `/808:fast [task description]`
...
**When to use vs `/808:quick`:**
- `/808:fast` — One-sentence tasks executable in under 2 minutes (typo, config change, small addition)
- `/808:quick` — Anything needing research, multi-step planning, or verification

---

### 42. Cross-AI Peer Review

**Command:** `/808:review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: Reviews MUST be consumable by `/808:plan-phase --reviews`
...
**Commands:** `/808:add-backlog <description>`, `/808:review-backlog`, `/808:plant-seed <idea>`
...
- REQ-BACKLOG-02: Phase directories MUST be created immediately so `/808:discuss-phase` and `/808:plan-phase` work on them
- REQ-BACKLOG-03: `/808:review-backlog` MUST support promote, keep, and remove actions per item
...
- REQ-SEED-02: `/808:new-milestone` MUST scan seeds and present matches
...
**Command:** `/808:thread [name | description]`

**Purpose:** Lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/808:pause-work` — no phase state, no plan context.
...
**Command:** `/808:pr-branch [target branch]`

**Purpose:** Create a clean branch suitable for pull requests by filtering out `.planning/` commits. Reviewers see only code changes, not 808 planning artifacts.
...
**Purpose:** Defense-in-depth security for 808's planning artifacts. Because 808 generates markdown files that become LLM system prompts, user-controlled text flowing into these files is a potential indirect prompt injection vector.
...
**2. Prompt Injection Guard Hook** (`808-prompt-guard.js`)
...
**3. Workflow Guard Hook** (`808-workflow-guard.js`)
PreToolUse hook that detects when Claude attempts file edits outside a 808 workflow context. Advises using `/808:quick` or `/808:fast` instead of direct edits. Configurable via `hooks.workflow_guard` (default: false).
...
**Purpose:** Auto-generate `DISCUSSION-LOG.md` during `/808:discuss-phase` for full audit trail of decisions made during discussion.
...
**Command:** `/808:forensics [description]`

**Purpose:** Post-mortem investigation of failed or stuck 808 workflows.
...
**Command:** `/808:milestone-summary [version]`
...
2. **Switch** — Change active workstream context for subsequent 808 commands
...
**Command:** `/808:discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**Part of:** `/808:new-project` and `/808:progress`

**Purpose:** Automatically detect UI-heavy projects and surface `/808:ui-phase` recommendation.
...
- REQ-UI-DETECT-03: System MUST suggest `/808:ui-phase` in next steps for UI-heavy phases
- REQ-UI-DETECT-04: System MUST NOT make `/808:ui-phase` mandatory
...
3. **Surface** — Include `/808:ui-phase` recommendation in next steps for UI-heavy phases
...
3. **Install** — Configure 808 for all selected runtimes in a single session
